...
print(f"Scenario {SCENARIO}: {TARGETS[SCENARIO]['name']}")
...
def elastic_energy(p_inner_flat):
    p = rebuild_rod(p_inner_flat)
...
# Cost function
def cost(p_inner_flat, target_shape):
    p = rebuild_rod(p_inner_flat)
    E_elastic = elastic_energy(p_inner_flat)
    E_targets = dist_targets(p, target_shape)
    return W1 * E_elastic + W2 * E_targets


# Intermediate Shape Generation (ISG)
...
for k, target_shape in enumerate(intermediary_shapes):
    result = minimize(cost, x, args=(target_shape,), method="BFGS",
                      options={"maxiter": 300, "gtol": 1e-6})
    x = result.x

    if k % max(1, len(intermediary_shapes) // 5) == 0 or k == len(intermediary_shapes) - 1:
        p_k = rebuild_rod(x)
        e_mean_shape = np.mean([
            np.sqrt(np.min(np.sum((p_k - t)**2, axis=1)))
            for t in s_desired
        ])
        e_mean_targets = np.mean([
            np.sqrt(np.min(np.sum((p_k - t)**2, axis=1)))
            for t in targets_final
        ])
        print(f"step {k + 1:2d}/{len(intermediary_shapes)}: "
              f"e_shape={e_mean_shape * 1000:.2f} mm, "
              f"e_targets={e_mean_targets * 1000:.2f} mm, "
              f"p(L)={p_k[-1].round(3)}")

print(f"\nComputation time: {time.time() - t_start:.3f} s")

p_final = rebuild_rod(x)
...
ax.scatter(targets_final[:, 0], targets_final[:, 1], targets_final[:, 2],
           color="red", s=80, zorder=5, label="Original targets")
